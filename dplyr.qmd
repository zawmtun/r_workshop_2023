...
From `surveys` data, filter mothers who gave birth to their last child at home and retain only columns `age`, `ancplace`, and `tetanus_vacc`
...
Let's check if the grouping is done correctly using `tabyl()`.
